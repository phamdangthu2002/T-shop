...
class ChiTietGioHangModel extends Database
{
    ...
    public function ChiTietGioHang__Add($magh, $masp, $soluong, $dongia)
    {

        $obj = $this->connect->prepare("INSERT INTO chitietgiohang(magh, masp, soluong, dongia) VALUES (?,?,?,?)");
        $obj->execute(array($magh, $masp, $soluong, $dongia));
    ...
    public function ChiTietGioHang__Check($magh, $masp, $makh)
    {
        $obj = $this->connect->prepare("SELECT * FROM chitietgiohang INNER JOIN giohang ON chitietgiohang.magh = giohang.magh WHERE giohang.magh=? AND masp=? AND makh = ? AND trangthai=?");
        $obj->setFetchMode(PDO::FETCH_OBJ);
        $obj->execute(array($magh, $masp, $makh, 1));
        if ($obj->rowCount() > 0) {
            return $obj->fetch();
        } else {
            return false;
